...
"""
Flask 聊天機器人應用程式
基於 chatbot_test.py 創建的獨立聊天 UI，支援 zhconv 中文轉換
...
@app.route('/convert_test', methods=['POST'])
def convert_test():
    """
    測試文字轉換功能
    """
    try:
        data = request.get_json()
        text = data.get('text', '')
        convert_type = data.get('convert_type', 's2tw')
        
        if not text:
            return jsonify({'error': '請提供要轉換的文字'}), 400
        
        converted_text = convert_text(text, convert_type)
...
if __name__ == '__main__':
    print("=" * 60)
    print("Flask 醫療聊天機器人應用程式 (支援 zhconv 中文轉換)")
    print("=" * 60)
    ...
    # print("🔄 支援中文轉換模式: 簡→繁(台)、簡→繁、簡→繁(港)、繁→簡、不轉換")
    print("=" * 60)
