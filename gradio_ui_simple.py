...
def process_image(input_image):
    ...
    try:
        ...
        info_text = "分割完成！白色區域代表檢測到的椎間盤組織"
...
# 創建範例文件列表（如果存在的話）
examples = []
if os.path.exists("/media/r300/1T/A30335/Disc/1 (3).png"):
    examples = [["/media/r300/1T/A30335/Disc/1 (3).png"]]

# 創建 Gradio 介面
interface = gr.Interface(
    fn=process_image,
    inputs=gr.inputs.Image(type="pil", label="上傳醫學影像"),
    outputs=[
        gr.outputs.Image(type="numpy", label="原始圖像"),
        gr.outputs.Image(type="numpy", label="第一類分割結果"),
        gr.outputs.Image(type="numpy", label="第二類分割結果"),
        gr.outputs.Textbox(label="處理狀態")
    ],
    title="椎間盤分割系統",
    description="上傳一張醫學影像，系統將自動進行椎間盤分割分析。白色區域代表檢測到的椎間盤組織。",
    examples=examples
)
